  ...
  return (
    <div className="pt-[68px]">
      ...
      <section className="relative hl-b border-black/10 overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: 'radial-gradient(circle at 50% 60%, rgba(242,239,231,0.5) 0%, rgba(242,239,231,0.92) 80%)' }} />
        <div className="relative px-6 md:px-10 py-20 md:py-32 text-center">
          <div className="font-mono text-[10px] tracking-[0.22em] uppercase text-[#1a1715]/60 mb-6">
          ...
          <p className="max-w-xl mx-auto font-mono text-[12px] leading-relaxed text-[#1a1715]/65 mt-10 uppercase tracking-[0.05em]">
            {D.drop.blurb}
          </p>
      ...
      <section className="hl-b border-black/10">
        ...
        />
        <div className="grid grid-cols-12 hl-t border-black/10 font-mono text-[11px]">
          <div className="col-span-12 hl-b border-black/10 grid grid-cols-12 px-6 md:px-10 py-4 tracking-[0.18em] uppercase text-[#1a1715]/40">
          ...
          {products.map((p, i) => (
            <div key={p.id} className="col-span-12 grid grid-cols-12 px-6 md:px-10 py-5 hl-b border-black/8 hover:bg-[#1a1715]/[0.02] transition-colors group" data-magnetic>
              <div className="col-span-1 text-[#1a1715]/50 tabular">{String(i + 1).padStart(2, '0')}</div>
              <div className="col-span-2 text-[#1a1715]/70 tracking-[0.1em]">{p.ref}</div>
              <div className="col-span-4">
                <div className="font-display text-lg leading-tight" style={{ fontWeight: 900, letterSpacing: '-0.02em' }}>{p.name}</div>
                <div className="text-[10px] text-[#1a1715]/40 tracking-[0.1em] uppercase mt-0.5">{p.sub}</div>
              </div>
              <div className="col-span-2 text-[#1a1715]/60 tracking-[0.1em] uppercase text-[10px]">
              ...
              <div className="col-span-2">
                ...
                <span className="text-[#1a1715]/70 tracking-[0.1em] uppercase text-[10px]">{p.stock}</span>
              </div>
      ...
      <section className="hl-b border-black/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 hl-t border-black/10">
          {[
            ...
            <div key={s.n} className={`p-8 md:p-10 ${i < 3 ? 'md:hl-r' : ''} hl-b border-black/10`}
              style={i < 3 ? { borderRight: '1px solid rgba(26,23,21,0.1)' } : undefined}>
              <div className="font-display tabular text-6xl text-[#1a1715]/15" style={{ fontWeight: 900, letterSpacing: '-0.04em' }}>{s.n}</div>
              <div className="font-mono text-[11px] tracking-[0.2em] uppercase text-[#1a1715] mt-6">{s.t}</div>
              <p className="font-mono text-[12px] text-[#1a1715]/55 mt-3 leading-relaxed">{s.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* sign up */}
      <section className="px-6 md:px-10 py-20 md:py-32 text-center hl-b border-black/10">
        <div className="font-eyebrow text-[#1a1715]/50 mb-6">DERNIÈRE ÉTAPE</div>
    ...
    <form onSubmit={submit} className="mt-12 max-w-xl mx-auto">
      <div className="flex hl-white-strong">
        <input
          ...
          data-cursor="text"
          className="flex-1 bg-transparent px-5 py-5 outline-none font-mono text-[14px] placeholder-[#1a1715]/30"
        />
        <button type="submit" className="px-8 py-5 bg-[#1a1715] text-[#F2EFE7] font-mono text-[12px] tracking-[0.22em] hover:bg-[#1a1715]/80 transition-colors" data-magnetic>
          {sent ? 'INSCRIT ✓' : 'M\'INSCRIRE →'}
        </button>
      </div>
      <div className="mt-4 font-mono text-[10px] tracking-[0.18em] text-[#1a1715]/40 uppercase">
        1 ÉMAIL · ZÉRO SPAM · RÉSILIATION 1 CLIC
      </div>
      {sent && (
        <div className="mt-6 font-mono text-[11px] text-[#1a1715]/70 tracking-[0.05em] uppercase" style={{ color: accent }}>
