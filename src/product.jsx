...
function ProductPage({ product, products, onBack, onAdd, onOpenCart, accent }) {
  ...
  return (
    <div className="pt-[88px]">
      ...
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-12 lg:col-span-7 hl-r border-white/10">
          <div className="relative aspect-[5/6]">
            ...
            {/* image index */}
            <div className="absolute top-4 right-4 font-mono text-[10px] tracking-[0.2em] text-white/70 bg-black/40 px-2 py-1 hl-white-strong tabular">
              {String(activeImg + 1).padStart(2, '0')} / 03
            </div>
          </div>
          {/* thumbs */}
          <div className="grid grid-cols-3 hl-t border-white/10">
            {[0, 1, 2].map((i) => (
              <button
                key={i}
                onClick={() => setActiveImg(i)}
                className={`aspect-square relative hl-r border-white/10 last:border-r-0 transition-opacity ${activeImg === i ? 'opacity-100' : 'opacity-50 hover:opacity-80'}`}
                data-magnetic
              >
                <image-slot
                  id={`pdp-${product.id}-${i}`}
        ...
        {/* RIGHT — product info */}
        <div className="col-span-12 lg:col-span-5 sticky top-[88px] self-start">
          <div className="p-6 md:p-10 hl-b border-white/10">
            <div className="flex items-center gap-2 mb-6">
              {product.tag && (
                <span className="font-mono text-[9px] tracking-[0.18em] px-2 py-1 bg-white text-black uppercase">
                  {product.tag}
                </span>
              )}
              {product.explicit && <span className="explicit-badge">EXPLICIT</span>}
              <span className="font-mono text-[10px] tracking-[0.2em] uppercase text-white/40 ml-auto">{product.ref}</span>
            </div>
            <h1 className="font-display leading-[0.92]" style={{ fontWeight: 900, fontSize: 'clamp(40px, 4.4vw, 68px)', letterSpacing: '-0.04em' }}>
              {product.name}
            </h1>
            <div className="font-mono text-[12px] tracking-[0.04em] uppercase text-white/55 mt-3">
              {product.sub}
            </div>
            <div className="mt-6 flex items-baseline justify-between hl-b border-white/10 pb-6">
              <div className="font-display tabular" style={{ fontWeight: 900, fontSize: '40px' }}>{product.price}€</div>
              <div className="font-mono text-[10px] tracking-[0.2em] uppercase text-white/40">TVA INCLUSE · LIV. INCLUSE FR</div>
            </div>
          </div>
          ...
          <div className="p-6 md:p-10 hl-b border-white/10">
            <div className="flex items-center justify-between mb-4">
              <div className="font-eyebrow text-white/50">TAILLE</div>
              <button className="font-mono text-[10px] tracking-[0.2em] uppercase text-white/40 hover:text-white" data-magnetic>
                GUIDE DES TAILLES ↗
              </button>
            </div>
            <div className="grid grid-cols-5 gap-2">
              {product.sizes.map((s) => (
                <button
                  key={s}
                  onClick={() => setSize(s)}
                  className={`py-3 font-mono text-[12px] tracking-[0.15em] transition-colors ${size === s ? 'bg-white text-black border border-white' : 'border border-white/15 hover:border-white/50'}`}
                  data-magnetic
                >
                  {s}
                </button>
          ...
          <div className="p-6 md:p-10 hl-b border-white/10">
            <div className="flex items-center gap-4 mb-4">
              <div className="font-eyebrow text-white/50">QUANTITÉ</div>
              <div className="flex items-center hl-white">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-10 h-10 hl-r border-white/15 font-mono hover:bg-white/5" data-magnetic>−</button>
                <span className="w-12 text-center font-mono tabular">{String(qty).padStart(2, '0')}</span>
                <button onClick={() => setQty(Math.min(8, qty + 1))} className="w-10 h-10 hl-l border-white/15 font-mono hover:bg-white/5" data-magnetic>+</button>
              </div>
            </div>
          ...
          <div className="p-6 md:p-10 space-y-5 font-mono text-[12px] text-white/70 leading-relaxed">
            <p>{product.desc}</p>
            ...
            <Accordion title="LIVRAISON & RETOURS">
              <p className="text-white/60">Expédition sous 5–10 jours après clôture du drop. Retours acceptés sous 14 jours, frais à la charge du client. Une fois la transmission terminée, les invendus partent au pilon.</p>
            </Accordion>
            <Accordion title="ENTRETIEN">
              <p className="text-white/60">Lavage 30°C envers, sans adoucissant. Séchage à plat. Ne pas repasser sur la sérigraphie.</p>
            </Accordion>
          </div>
        </div>
      </div>
      ...
      {/* related */}
      <div className="hl-t border-white/10">
        <SectionHeader
          index="—"
...
function Accordion({ title, children }) {
  ...
  return (
    <div className="hl-t border-white/10 pt-4">
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between font-mono text-[11px] tracking-[0.2em] uppercase text-white hover:text-white/80" data-magnetic>
        <span>{title}</span>
...
function ProductCardMini({ p, idx }) {
  return (
    <div className="aspect-[4/5] product-card relative hl-r hl-b border-white/10">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 hl-t border-white/10 bg-black/40 backdrop-blur-sm flex items-end justify-between">
        <div>
          <div className="font-mono text-[10px] tracking-[0.18em] uppercase text-white/50">{p.ref}</div>
